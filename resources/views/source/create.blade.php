            ...
            <div class="col-md-12">
                <form class="" action="{{url("/donor/{$donor_name}/sources/store")}}" method="post">
                    ...
                    <div class="form-group{{ ($errors->has('category_id')) ? $errors->first('category_id') : '' }}">
                        <input type="text" name="category_id" class="form-control" placeholder="ID категория на mobzilla.su">
                        {!! $errors->first('category_id','<p class="help-block">:message</p>') !!}
                    </div>
                    <div class="form-group{{ ($errors->has('category_id_2')) ? $errors->first('category_id_2') : '' }}">
                        <input type="text" name="category_id_2" class="form-control" placeholder="ID категории на cifrolink.com">
                        {!! $errors->first('category_id_2','<p class="help-block">:message</p>') !!}
                    </div>
